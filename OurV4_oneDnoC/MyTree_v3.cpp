...
bool sortCmp1(pair<int, int> a, pair<int, int> b )
{
...
string MyTree::TreeCamGen(int** subgraphTree, int level)
{
  ...
  for(curlevel = level-2; curlevel >= 1; curlevel--)// we don't need to adjust leaf nodes in the lowest level
  {
    ...
    for(i = 1; i<=subgraphTree[uplevel][0]; i++)
    {
      ...
      /***Add by wangtao on 6/14th/2015***/
      vector<int> childrenNumList;
      int flag = 1;
      for(list<int>::iterator j = adj[parentNode].begin(); j != adj[parentNode].end(); j++)
      {
        int children_num = ChildrenNum(*j);
      ...
      { 
        ...
        for(list<int>::iterator j = adj[parentNode].begin(); j != adj[parentNode].end(); j++)
        {
          pair<int, int> apair = make_pair(*j, ChildrenNum(*j));
          tmp.push_back(apair);
        }
        sort(tmp.begin(), tmp.end(), sortCmp1);
        list<int> list_tmp;
...
{
  return a.second > b.second;// changed from < to > on 6/14th/2015
}
