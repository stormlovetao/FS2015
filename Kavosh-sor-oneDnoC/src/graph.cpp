...
graph canon[MAXN * MAXM]; // In graph.h #define MAXN 100; In nauty.h #define MAXM ((MAXN+WORDSIZE-1)/WORDSIZE), and wordsize = 32, so MAXM = 4.
graph * nauty_g;//In nauty.h typedef setword set,graph; we can take setword as int here
int* lab;
int* ptn;
int* orbits;
...
Graph::Graph(const int n, const int k) {
	...
	options.defaultptn = FALSE;//modified by wangtao on 1/8/2013
...
void Graph::E_temp_print(){
	register int i,j;
	for (i = 1; i <= nV; i++)
	{
		printf("E_temp%d size is (%d) : ",i, E_temp[i].size() );
...
void Graph::addEdgeAdjMat(vertex u, vertex v) {
	...
	//adjMat[(rowSize*v) + (u>>3)] |= (1 << (u % 8));//add by wangtao on 19th/12/2013 randomGenerator.cpp
}
...
void Graph::deleteEdgeAdjMat(vertex u, vertex v) {
	adjMat[(rowSize*u) + (v>>3)] &= (~(1 << (v % 8)));
	//adjMat[(rowSize*v) + (u>>3)] &= (~(1 << (u % 8)));//add by wangtao on 19th/12/2013
}
...
void Graph::addEdge(vertex u, vertex v) {
	...
	adjMat[(rowSize*u) + (v>>3)] |= (1 << (v % 8));
	adjMat[(rowSize*v) + (u>>3)] |= (1 << (u % 8));//add by wangtao on 4/20th/2015, to make sure the input network is undirected.
}

/****************************************************************
****************************************************************/

void Graph::adjM_print(){
	...
	for(i = 1; i <= nV; i++)
	{
		for(j = 0; j < rowSize; j++)
		{
			temp = adjMat[rowSize*i + j];
			for(offset = 0; offset <= 7; offset++)
			{
				int val = (int)((temp & (1<<offset)) >>offset );
				printf("%d ", val);
			}
			printf("  ");
		}
...
void Graph::Finalize() {
	...
	for(i = 1; i <= nV; i++) {
		...
		for(j = 0; j < E_temp[i].size(); j++) {
			...
			if(isConnected(i, E_temp[i][j])) {//isConnected(i, E_temp[i][j])   //change by wangtao
				//printf("%d, %d\n", E_temp[i][j], isConnected(i, E_temp[i][j]) );
				//degree[degInd] = i;
				degInd++;
				nEd++;
				//printf("nEd = %d\n", nEd);
...
void Graph::Classify(vertex **subgraph, int level) {
	...
	for (i = 0; i < subgraphSize; i++) {
		gv = GRAPHROW(nauty_g, i, M);//gv = nauty_g + i*M    M = (subgraphSize + WORDSIZE - 1)/WORDSIZE
		EMPTYSET(gv, M);		
		for(j = 0; j < subgraphSize; j++) {
			...
			{
				ADDELEMENT(gv, j);//gv[j>>5] |= bit[j&0x1F]
			}
		}
	}
    ...
    /***/
	nauty(nauty_g, lab, ptn, NULL, orbits, &options, &stats,  workspace, 160*MAXM, M, subgraphSize, canon);
...
void Graph::DFSmain(Node * cur, char * str, int lev) {
	if(!cur->left && !cur->right) {
		/**shut down the output by wangtao on 2/25th/2014**/
		//print_adjMatrix(str); 
		Leaf * leaf = (Leaf *)cur;
		head++;
